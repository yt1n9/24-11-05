...
function draw() { //開始畫圖，每秒會進入draw()會執行60次
  ...
  //translate(mouseX,mouseY) //設定原點(0,0)在滑鼠游標位置
...
let r = map(sin(frameCount),-1,1,200,255)  //對應
let g = map(cos(frameCount/2),-1,1,20,100)  //對應
let b = map(sin(frameCount/4),-1,1,200,255)  //對應
  random()  //抽一個亂數，亂數的值會介於0(包含)~1(不包含)間
  stroke(r,g,b)
  for(let i=0;i<33;i=i+1){
    rotate(sin(angel-i*2)*40) //三角函數sin的值介於-1~1間,*40之後,旋轉角度介於-40~40
    rect(0,0,100+(mouseX/10)-i*3,100+(mouseX/10)-i*3,20) //劃一個方形(600,600) ，圓弧在四角寬高為100
  }
